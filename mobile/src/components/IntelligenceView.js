...
export function IntelligenceView({ data, source }) {
  if (!data) return null;
    ...
    <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        <FadeInSection delay={0}>
          <GlassCard style={styles.card}>
            ...
            <View style={styles.metaRow}>
              {source.author && (
                <Text style={styles.metaText}>{source.author}</Text>
              )}
              {source.duration && (
                <Text style={styles.metaText}>{source.duration}</Text>
              )}
              {source.views && (
                <Text style={styles.metaText}>{source.views} views</Text>
              )}
            </View>
          </GlassCard>
        </FadeInSection>
      ...
      {intel?.actionItems?.length > 0 && (
        <FadeInSection delay={700}>
          <GlassCard style={styles.card}>
            <SectionHeader icon="rocket" title="Action Items" delay={750} />
            {intel.actionItems.map((item, i) => (
              <FadeInSection key={i} delay={780 + i * 50}>
